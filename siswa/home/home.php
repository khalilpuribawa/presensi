...
while($lokasi = mysqli_fetch_array($result)){
    $latitude_sekolah = $lokasi['latitude'];
    $longtitude_sekolah = $lokasi['longtitude'];
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="card text-center h-100">
                    ...
                    <div class="card-body">
                        ...
                        $check_presensi_masuk = mysqli_query($connection, "SELECT * FROM presensi WHERE id_siswa = '$id_siswa' AND tanggal_masuk = '$tanggal_hari_ini'");
                        ?>

                        <?php if(mysqli_num_rows($check_presensi_masuk) === 0) { ?>
                        ...
                        <form method="POST" action="<?= base_url('siswa/presensi/presensi_masuk.php') ?>">
                            <input type="hidden" name="latitude_siswa" id="latitude_siswa">
                            <input type="hidden" name="longtitude_siswa" id="longtitude_siswa">
                            <input type="hidden" value="<?= $latitude_sekolah ?>" name="latitude">
                            <input type="hidden" value="<?= $longtitude_sekolah ?>" name="longtitude">
                            ...
                            <button type="submit" class="btn btn-primary mt-3" name="tombol_masuk">Masuk</button>
                        </form>
                        <?php }else{ ?>
                        <i class="fa-solid fa-circle-check fa-4x text-success"></i>
                        <h4 class="my-3 ">Anda telah melakukan <br>absen masuk hari ini</h4>
                        <?php } ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="card text-center h-100">
                    ...
                    <div class="card-body">
                        <?php 
                        $ambil_data_presensi = mysqli_query($connection, "SELECT * FROM presensi WHERE id_siswa = '$id_siswa' AND tanggal_masuk = '$tanggal_hari_ini'");
                        ?>
                        ...
                        <i class="fa-solid fa-circle-xmark fa-4x text-danger"></i>
                        <h4 class="my-3 ">Silahkan melakukan absen masuk <br>terlebih dahulu</h4>
                        <?php }else{ ?>

                        <?php while($check_presensi_keluar = mysqli_fetch_array($ambil_data_presensi)) { ?>
                        <?php if(($check_presensi_keluar['tanggal_keluar']) && $check_presensi_keluar['tanggal_keluar'] == '0000-00-00') { ?>
                        <div class="parent-date">
                        ...
                        <form method="POST" action="<?= base_url('siswa/presensi/presensi_keluar.php') ?>">
                            <input name="id" type="hidden" value="<?= $check_presensi_keluar['id'] ?>">
                            <input type="hidden" name="latitude_siswa" id="latitude_siswa">
                            <input type="hidden" name="longtitude_siswa" id="longtitude_siswa">
                            <input type="hidden" value="<?= $latitude_sekolah ?>" name="latitude">
                            <input type="hidden" value="<?= $longtitude_sekolah ?>" name="longtitude">
...
function waktuKeluar() {
    ...
    document.getElementById("jam_keluar").innerHTML = waktu
        .getHours();
...
function showPosition(position) {
    $('#latitude_siswa').val(position.coords.latitude);
    $('#longtitude_siswa').val(position.coords.longitude);
}
</script>
